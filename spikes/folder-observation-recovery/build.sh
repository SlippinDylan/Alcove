...
# build.sh — Phase 0.5C3 observer path and recovery build
# Compiles the recovery probe binary from candidate sources directly.
# Usage: bash build.sh
...
echo "=== Phase 0.5C3 — Observer Recovery Build ==="
...
xcrun vtool -show-build "${PROBE_BINARY}" 2>/dev/null | head -10 || echo "(vtool not available)"
echo ""
echo "Code signature:"
