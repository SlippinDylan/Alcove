...
# test.sh — Phase 0.5C3 observer path and recovery tests
# Builds probe and test harness, runs tests.
# Usage: bash test.sh

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BUILD_DIR="${SCRIPT_DIR}/build"
PROBE_BINARY="${BUILD_DIR}/recovery-probe"
TEST_BINARY="${BUILD_DIR}/recovery-tests"
...
echo "=== Phase 0.5C3 — Observer Recovery Tests ==="
echo ""
...
# Artifact inspection on final binary.
echo ""
echo "=== Final Binary Inspection ==="
echo ""
echo "Architecture:"
file "${PROBE_BINARY}"
echo ""
echo "Build version:"
xcrun vtool -show-build "${PROBE_BINARY}" 2>/dev/null | head -15 || echo "(vtool not available)"
echo ""
echo "Dynamic libraries:"
otool -L "${PROBE_BINARY}" | head -10
echo ""
echo "Code signature:"
codesign -dv "${PROBE_BINARY}" 2>&1 | head -5 || echo "(codesign inspection)"
