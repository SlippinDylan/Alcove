...
# test.sh — Alcove Spike 0.5A DispatchSource Folder Observer Tests
# Compiles and runs the automated test harness.
# Usage: bash test.sh

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BUILD_DIR="${SCRIPT_DIR}/build"
TEST_BINARY="${BUILD_DIR}/folder-observer-tests"
...
echo "=== Alcove Spike 0.5A — DispatchSource Folder Observer Tests ==="
echo ""
...
xcrun swiftc \
    ...
    -o "${TEST_BINARY}" \
    "${SCRIPT_DIR}/Sources/FolderObservationRecord.swift" \
    "${SCRIPT_DIR}/Sources/FolderObserverError.swift" \
    "${SCRIPT_DIR}/Sources/DispatchSourceFolderObserver.swift" \
