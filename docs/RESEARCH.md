...
| Choosing DispatchSource, FSEvents, or a combination for Alcove's active-tab observation | **Prototype required** | Public API availability does not establish event coverage, lifecycle recovery, removable-volume behavior, resource cost, or latency for Alcove's use case; Spike 0.5 compares them |
...
| INF-03 | Either `DispatchSource` (kqueue) or FSEvents may be suitable for observing the active tab's mapped directory | Both APIs are public and have different event scope, resource, path-lifecycle, removable-volume, and coalescing behavior; Spike 0.5 selects the mechanism from measurements and failure behavior |
| INF-04 | `NSGlassEffectView` applied to portal chrome (tab bar, controls) but not the file content canvas will follow Apple's Liquid Glass guidance | Apple explicitly says glass is for chrome/navigation/controls, not content areas |
...
| SP-07 | Compare `DispatchSourceFileSystemObject` and FSEvents for active-directory observation | Either mechanism may miss lifecycle events or impose unsuitable resource/coalescing behavior | Spike 0.5 records add/remove/rename, missing-folder, removable-volume, symlink, overhead, teardown, and reattachment behavior, then selects the mechanism and fallback policy |
