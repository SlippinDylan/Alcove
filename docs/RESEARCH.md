...
| SP-06 | `NSGlassEffectView` on portal chrome plus the opaque Reduce Transparency path | Either path may have rendering, contrast, accessibility, layout, or selected-window-level issues | Verify acceptable Liquid Glass and opaque accessibility behavior on macOS 26+; failure requires an explicit visual-scope or architecture decision |
