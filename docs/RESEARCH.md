...
| SP-06 | Liquid Glass and Frosted Glass Portal surfaces plus the opaque Reduce Transparency path | Any path may have rendering, contrast, accessibility, layout, or selected-window-level issues | Verify `NSGlassEffectView`, semantic behind-window `NSVisualEffectView`, and opaque accessibility behavior on macOS 26+; failure requires an explicit visual-scope or architecture decision |
