...
| At Alcove's selected desktop window level, folder controls inside a small `NSGlassEffectView.contentView` remained hit-testable but rendered fully transparent; a visible host fill and foreground control overlay are required | **Observed in production UI** | User-provided macOS 26 screenshots on 2026-09-12 after commits `36ac213` and `98cd17d` |
