...
| `NSScreen.screens[0]` is the menu-bar primary display, while `NSScreen.main` is the display containing the keyboard-focus window | **Confirmed by Apple** | [Apple Developer — NSScreen.screens](https://developer.apple.com/documentation/appkit/nsscreen/screens); [Apple Developer — NSScreen.main](https://developer.apple.com/documentation/appkit/nsscreen/main) |
| `NSWindow.setFrame` uses the frame supplied by the app, while `constrainFrameRect(_:to:)` does not provide Alcove's complete four-edge and inter-window collision policy | **Confirmed by Apple / architecture consequence** | [Apple Developer — setFrame](https://developer.apple.com/documentation/appkit/nswindow/setframe(_:display:)); [Apple Developer — constrainFrameRect](https://developer.apple.com/documentation/appkit/nswindow/constrainframerect(_:to:)) |
...
| `NSCollectionViewDelegate` can redirect a validated drop to a specific item with `.on`; validation is called repeatedly during a drag and should remain free of filesystem IO | **Confirmed by Apple** | [Apple Developer — validateDrop](https://developer.apple.com/documentation/appkit/nscollectionviewdelegate/collectionview(_:validatedrop:proposedindexpath:dropoperation:)); [Apple Developer — DropOperation.on](https://developer.apple.com/documentation/appkit/nscollectionview/dropoperation/on) |
| `NSView.menu(for:)` is the context-sensitive menu hook, and `NSMenuItemValidation` enables or disables individual commands against current state | **Confirmed by Apple** | [Apple Developer — menu(for:)](https://developer.apple.com/documentation/appkit/nsview/menu(for:)); [Apple Developer — NSMenuItemValidation](https://developer.apple.com/documentation/appkit/nsmenuitemvalidation) |
| Finder reveal, direct AirDrop, and opening selected URLs with Apple Terminal are supported through public AppKit APIs | **Confirmed by Apple** | [Apple Developer — activateFileViewerSelecting](https://developer.apple.com/documentation/appkit/nsworkspace/activatefileviewerselecting(_:)); [Apple Developer — sendViaAirDrop](https://developer.apple.com/documentation/appkit/nssharingservice/name/sendviaairdrop); [Apple Developer — open URLs with an application](https://developer.apple.com/documentation/appkit/nsworkspace/open(_:withapplicationat:configuration:completionhandler:)) |
| `NSTextFieldDelegate` can intercept Return/Escape command selectors, while `controlTextDidEndEditing` commits focus-loss editing; coordinated renames use `.forMoving` plus balanced move notifications | **Confirmed by Apple** | [Apple Developer — control(_:textView:doCommandBy:)](https://developer.apple.com/documentation/appkit/nscontroltexteditingdelegate/control(_:textview:docommandby:)); [Apple Developer — controlTextDidEndEditing](https://developer.apple.com/documentation/appkit/nscontroltexteditingdelegate/controltextdidendediting(_:)); [Apple Developer — forMoving](https://developer.apple.com/documentation/foundation/nsfilecoordinator/writingoptions/formoving); [Apple Developer — item(at:didMoveTo:)](https://developer.apple.com/documentation/foundation/nsfilecoordinator/item(at:didmoveto:)) |
| A file resource identifier compares filesystem resources but also equates hard links, so safe rename preflight must combine it with the volume's case-sensitive-name capability and normalized name equality | **Confirmed by Apple / architecture consequence** | [Apple Developer — fileResourceIdentifierKey](https://developer.apple.com/documentation/foundation/urlresourcekey/fileresourceidentifierkey); [Apple Developer — volumeSupportsCaseSensitiveNamesKey](https://developer.apple.com/documentation/foundation/urlresourcekey/volumesupportscasesensitivenameskey) |
| `NSWorkspace.duplicate` duplicates multiple URLs asynchronously in the same manner as Finder and returns a source-to-copy URL mapping | **Confirmed by Apple** | [Apple Developer — duplicate](https://developer.apple.com/documentation/appkit/nsworkspace/duplicate(_:completionhandler:)) |
| Finder's installed scripting dictionary exposes an item's `information window`, and the standard `open information window of …` Apple Event opens the real Finder UI. Apple requires `NSAppleEventsUsageDescription` for apps that control another app; `NSAppleScript.executeAppleEvent` supports invoking a fixed handler with descriptor arguments | **Confirmed by Finder's macOS 26.5 scripting dictionary and Apple documentation** | [Apple Developer — NSAppleEventsUsageDescription](https://developer.apple.com/documentation/bundleresources/information-property-list/nsappleeventsusagedescription); [Apple Developer — executeAppleEvent](https://developer.apple.com/documentation/foundation/nsapplescript/executeappleevent(_:error:)) |
| macOS `ditto -c -k --sequesterRsrc --keepParent` creates a PKZip in the same shape as Finder Compress for one directory; `ditto` rejects multiple archive sources, so multi-item ZIP requires a clean staging payload. Shipping Swift projects independently use the same fixed executable and argument set | **Confirmed by the macOS 26.5 `ditto(1)` manual, local archive/extract spike, and open-source implementations** | [Pearcleaner implementation](https://github.com/alienator88/Pearcleaner/blob/7724df7111bff82ae243301cf701992ef05ecf19/Pearcleaner/Logic/Logic.swift); [Vorssaint implementation](https://github.com/vorssaint/vorssaint-utils/blob/f7ab637c93ad63f95cf310ebc52da383ad3826e8/Sources/Vorssaint/Services/Notch/NotchFileToolsSupport.swift); [KlipKlik multi-item staging](https://github.com/d-sanoj/KlipKlik/blob/bd8dd0bd061cdab3eac947f40c2fc29b1d8558ad/Sources/KlipKlik/Core/Shelf/ShelfActions.swift) |
...
| `NSGlassEffectView` is available on macOS 26.0 and later; macOS 27 Golden Gate continues the macOS SDK line, so a 26.0 deployment target supports both generations without an older material branch | **Confirmed by Apple** | [Apple Developer — NSGlassEffectView](https://developer.apple.com/documentation/appkit/nsglasseffectview); [Apple Developer — macOS 27 release notes](https://developer.apple.com/documentation/macos-release-notes/macos-27-release-notes) |
...
| At Alcove's selected desktop window level, folder controls inside a small `NSGlassEffectView.contentView` remained hit-testable but rendered fully transparent; this evidence led production Portal navigation to remove its control-group Glass and use a direct Tab strip with separators | **Observed in production UI / current design consequence** | User-provided macOS 26 screenshots on 2026-09-12 after commits `36ac213`, `98cd17d`, and `3345920` |
...
| Apple recommends standard materials for content-layer backgrounds. On macOS, `NSVisualEffectView` provides translucency, blur, and vibrancy; `.behindWindow` samples the desktop or other windows, and `.active` keeps the backdrop active | **Confirmed by Apple** | [Apple HIG — Materials](https://developer.apple.com/design/human-interface-guidelines/materials); [Apple Developer — NSVisualEffectView](https://developer.apple.com/documentation/appkit/nsvisualeffectview); [Apple Developer — behindWindow](https://developer.apple.com/documentation/appkit/nsvisualeffectview/blendingmode-swift.enum/behindwindow); [Apple Developer — active](https://developer.apple.com/documentation/appkit/nsvisualeffectview/state-swift.enum/active) |
| With `.canJoinAllSpaces`, both `NSGlassEffectView` and behind-window `NSVisualEffectView` temporarily render a gray fallback while WindowServer rebinds the backdrop during Space transitions. Removing `.stationary` or using normal window level does not prevent it; removing `.canJoinAllSpaces` does, while `.moveToActiveSpace` causes a visible pop-in. A plain alpha-composited surface stays stable | **Observed on real macOS 26 hardware** | User-provided screenshots and timestamped window/material diagnostics on 2026-09-15; Apple documents individual APIs but does not guarantee backdrop continuity across Spaces |
| `NSWindow.hasShadow` is Boolean rather than a strength control, and `invalidateShadow()` recomputes the shadow after the window shape changes | **Confirmed by Apple / product consequence** | [Apple Developer — hasShadow](https://developer.apple.com/documentation/appkit/nswindow/hasshadow); [Apple Developer — invalidateShadow](https://developer.apple.com/documentation/appkit/nswindow/invalidateshadow()) — Alcove exposes an on/off switch and does not synthesize shadow intensity |
