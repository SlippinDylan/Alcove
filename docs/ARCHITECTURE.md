...
- `ApplicationUpdateController` — owns the single long-lived Sparkle updater, maps the build's explicit stable/alpha/beta channel to allowed appcast channels, and provides shared Check for Updates actions without leaking Sparkle into `AlcoveCore`
- `ApplicationSettingsWindowController` — owns the preference-style General/Style/Advanced/About window. General adapts `SMAppService.mainApp` for launch-at-login registration and persists a Follow System/English/Simplified Chinese/Traditional Chinese app-language choice; confirming the prompt follows the normal asynchronous termination-save path, then `ApplicationRelaunchController` opens a new app instance from the current bundle URL. Style stores global content size, static-background transparency, spacing, corner radius, and shadow in `UserDefaults`, with a separator between option rows; Advanced presents position repair and layout backup import/export; About reads version metadata and the compiled Icon Composer application icon and invokes the shared updater
...
- No Core Data or SQLite. Versioned Portal state remains JSON; `UserDefaults` is the sole durable source for application-global appearance and the app-language choice, while the v12 Portal envelope stores only per-Portal state such as sort and tint.
...
- AppKit strings use `en`, `zh-Hans`, and `zh-Hant` bundle resources. English is the development region and fallback for every other system language. The General settings language selector stores its explicit choice in `UserDefaults`, removes that override for Follow System, and can terminate and automatically reopen the current app bundle so the new process resolves one consistent bundle localization
