...
- AppKit strings use `en`, `zh-Hans`, and `zh-Hant` bundle resources. English is the development region and fallback for every other system language. The General settings language selector stores its explicit choice in `UserDefaults`, applies the corresponding per-app language preference on the next launch, and removes that override for Follow System so the whole process resolves one consistent bundle localization
