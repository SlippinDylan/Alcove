...
- Single-item Get Info opens one reusable AppKit inspector window. `FileInspectorLoader` reads the basic URL metadata off the main actor; regular-file size is part of that snapshot, while ordinary-folder size uses a cancellable deep `FileManager.DirectoryEnumerator`. Enumeration includes hidden/package contents, never follows symlink descendants, fails rather than reporting a knowingly partial total, and is cancelled when the window closes or switches items.
