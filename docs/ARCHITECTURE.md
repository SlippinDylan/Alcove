...
- `FileTransferPlan` is the validation boundary for Finder drops. The transfer actor runs it off the main actor before any mutation, rejecting a source already in the destination, duplicate destination names, existing targets, and a directory entering its own resolved descendant. AppKit's synchronous drag callbacks only inspect pasteboard structure and the requested operation; they never perform filesystem IO.
