...
| Contextual Get Info with one selected item | Open Alcove's read-only native inspector with name, localized kind, location, created/modified dates, and size |
...
| FR-27 | Right-clicking an item presents a native validated contextual menu while following Finder selection semantics. It provides Open, Quick Look, Show in Finder, single-item read-only Get Info, single-item inline Rename, Finder-style Duplicate through `NSWorkspace`, Move to Trash, direct AirDrop, newline-separated absolute-path copying, and Apple Terminal opening; right-clicking blank grid space neither displays a file menu nor changes selection. Get Info asynchronously loads metadata and cancellable non-symlink-traversing folder size; Return also begins single-item rename; invalid/conflicting names never overwrite, Escape cancels, and successful rename/duplicate actions retain the returned URL selection | MVP |
...
| AC-25 | File and folder context menus preserve or replace selection like Finder, expose only valid native actions, keep blank-space selection unchanged, and route multi-item Finder, AirDrop, path-copy, Terminal, Quick Look, open, duplicate, and Trash actions in stable grid order. Single-item Get Info reports read-only metadata and a cancellable accurate folder size without symlink traversal; single-item Return/contextual rename edits inline, preserves the file extension selection, rejects invalid/conflicting names, cancels with Escape, and retains selection after the path changes; Duplicate uses Finder naming and selects the returned copies | FR-27 |
