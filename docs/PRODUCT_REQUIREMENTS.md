...
| G-5 | Require macOS 26 or later and offer native Liquid Glass and Frosted Glass backgrounds |
...
| FR-12 | All Portals share one global content-size preset, one global Liquid Glass/Frosted Glass background type, and one global five-step material level; each Portal retains its own tint. Changing size is preflighted for every Portal before any frame changes. Lightweight separators distinguish the top controls and bottom path row without additional capsule materials. Clicking the top controls or draggable background activates the Portal just like clicking its grid or path row. | MVP |
| FR-13 | Require macOS 26 or later. Liquid Glass uses `NSGlassEffectView`; Frosted Glass uses one active `.underWindowBackground` `NSVisualEffectView` with `.behindWindow` blending and tint below content. Reduce Transparency overrides either type with an opaque accessibility surface | MVP |
...
| FR-25 | Advanced settings exports a stable versioned JSON layout backup containing global Portal appearance, including background type, and portable per-Portal layout state, but never launch-at-login. Import strictly validates the whole document and, after confirmation, replaces rather than merges the current layout. Version 1 backups without the additive background-type key default to Liquid Glass. The replacement is preflighted against the fresh primary display and persisted once before runtime windows change; any validation or save failure leaves the current runtime layout untouched. | MVP |
...
| AC-11 | The app binary declares macOS 26.0 as its minimum system; global background changes update every Portal between native Liquid Glass and Frosted Glass without changing per-Portal tint, and Reduce Transparency uses the opaque accessibility surface | FR-12, FR-13 |
...
- Native Liquid Glass and Frosted Glass on macOS 26 and macOS 27, with an opaque Reduce Transparency path
