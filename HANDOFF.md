...
**Current phase:** Phase 0 manual/system evidence remains open while replaceable development defaults unblock production work. Production Slices 1–9 now provide the universal AppKit shell, folder access, multi-tab portals, Finder-style interaction, transactional creation, Quick Look, atomic v2 placement persistence, display/wake recovery, and active-tab FSEvents refresh/recovery. Removable, ejectable, and network-volume folders remain outside product scope; hardening, an actual macOS 15 runtime, controlled TCC denial, and human GUI behavior remain open.
...
Production Slice 9 added the sole production FSEvents observer with `FileEvents`,
`WatchRoot`, and `UseCFTypes`. Only the active tab is observed. Ordinary records debounce
into full snapshot reloads; recovery flags stop the old stream, revalidate the supported
location and device/inode, restart before enumeration when unchanged, and reject replaced
roots. Portal close and tab switches stop the old generation. AlcoveCore passes 110 tests
and the hosted app passes 97 tests, including real FSEvents-to-grid refresh. Locate Folder
UI and controlled TCC denial remain open.

### Earlier Slice 8 placement
...
Production Slice 7 added Space handling for the grid's ordered selection, a dedicated
window responder that explicitly owns `QLPreviewPanel` data-source/delegate references,
second-Space dismissal, tab-switch relinquishment, and exact responder restoration on
detach. Thirty-four Core tests and 56 hosted tests pass; the unsigned Release app remains
universal arm64/x86_64 with macOS 15.0 minimum. Real preview rendering, carousel
navigation, focus handoff, and desktop-level behavior remain unverified.

### Earlier Slice 6 tabs

Production Slice 6 added the ordered accessible TabBar, validated folder addition,
persist-before-UI tab selection and closure, final-tab Portal confirmation, and
per-tab runtime selection/scroll restoration with one reusable grid. Thirty-four
Core tests and 46 hosted tests pass. Manual TabBar interaction remains unverified.

### Earlier Slice 5 persistence

Production Slice 5 added the validated non-Codable Portal aggregate, separate v1
Codable DTOs, readable snake_case JSON, same-directory atomic PortalStore,
startup restoration, save-before-present creation, and serialized frame writeback.
Thirty-four Core tests and 38 hosted tests pass. Manual relaunch remains unverified.

### Earlier Slice 4 portal creation
...
Command-A, double-click, Command-O, and Command-Down opening, plus explicit
Return no-op behavior. Seventeen Core tests and 22 hosted tests pass. Real desktop
focus and NSWorkspace opening remain manual.

### Earlier Slice 2 minimal portal
